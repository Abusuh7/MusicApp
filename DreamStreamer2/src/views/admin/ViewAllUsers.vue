      ...
      <div class="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
          ...
          <table class="w-full table-auto">
            <thead>
              <tr class="bg-gray-200 text-left">
                <th class="min-w-[220px] py-4 px-4 font-medium text-black">Username</th>
                <th class="min-w-[150px] py-4 px-4 font-medium text-black">Email</th>
                <th class="min-w-[150px] py-4 px-4 font-medium text-black">Role</th>
                <th class="min-w-[100px] py-4 px-4 font-medium text-black">Status</th>
                <th class="min-w-[150px] py-4 px-4 font-medium text-black">Created Date</th>
            ...
            <tbody>
              ...
              >
                ...
                <td class="py-5 px-4">{{ getAttribute(user.Attributes, 'email') || 'N/A' }}</td>
                <td class="py-5 px-4">{{ getRoleName(getAttribute(user.Attributes, 'custom:role')) }}</td>
                <td class="py-5 px-4">
                  <p
                    class="inline-flex rounded-full py-1 px-3 text-sm font-medium"
                    :class="{
                      'bg-green-100 text-green-500': user.UserStatus === 'CONFIRMED',
                ...
                <td class="py-5 px-4">
                  <div class="flex space-x-3">
                    <button @click="openEditModal(user)" class="text-blue-500 hover:text-blue-700">
                      <i class="fa-solid fa-pen-to-square"></i>
                    </button>
                    <button @click="toggleDelete(user.Username)" class="text-red-500 hover:text-red-700">
        ...
        <div v-if="showEditModal" class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
          <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
            <h3 class="text-xl font-semibold mb-4">Edit User: {{ selectedUser.username }}</h3>
            <form @submit.prevent="updateUser">
              <!-- Username (Disabled) -->
              <div class="mb-4">
                <label for="edit-username" class="block text-gray-700 font-bold mb-2">Username:</label>
                <input
                  type="text"
                  id="edit-username"
  ...
  const updateUser = async () => {
    try {
      const requestBody = {
        ...
        attributes: [
          { name: 'email', value: selectedUser.value.email },
          { name: 'custom:role', value: selectedUser.value.role }
        ]
      };
