  ...
  <div class="container">
    ...
    >
      <div class="modal">
        ...
        <div class="scrollable-song-list">
          <div
            v-for="song in filteredSongsNotInAlbum"
            :key="song.id"
            class="scrollable-song-item"
          >
            <img
    ...
    >
      <div class="modal">
        ...
        <div class="mb-4">
          <label for="albumName" class="block text-sm font-medium text-gray-700"
            >Album Name</label
          >
          <input
            type="text"
        ...
        <button @click="updateAlbumName" class="update-btn">
          Update Album Name
        </button>
        <div class="flex">
...
const albumSongs = ref([]); // The songs already in the album
...
const songs = ref([]); // All available songs
...
// Update album name where the album_id, new_album_name are passed as in body
const updateAlbumName = async () => {
  try {
    const response = await fetch(
      `https://4tvu26kjyf.execute-api.ap-southeast-1.amazonaws.com/dev/updatePersonalAlbum`,
      {
        method: "PUT",
        body: JSON.stringify({
          album_id: albumId.value,
          new_album_name: editedAlbumName.value,
        }),
      }
    );
    if (!response.ok) {
      throw new Error("Error updating album name");
    }
    fetchAlbum(albumId.value); // Refresh the album details
    closeEditModal();
  } catch (error) {
    console.error("Error updating album name:", error);
    errorMessage.value = "Error updating album name.";
    return;
  }
}; 

// Delete the album where the album_id is passed as in body
const deleteAlbum = async () => {
  try {
    const response = await fetch(
      `https://4tvu26kjyf.execute-api.ap-southeast-1.amazonaws.com/dev/deleteFullAlbum`,
      {
        method: "DELETE",
        body: JSON.stringify({
          album_id: albumId.value,
    ...
    // Redirect to the user's library after deletion
    router.push("/library");
  } catch (error) {
...
const filteredSongs = computed(() => {
  return songs.value.filter(
    (song) =>
      ...
      song.artist_name
        .toLowerCase()
        .includes(songSearchQuery.value.toLowerCase())
  );
});

// Filter songs to only show those not already in the album
const filteredSongsNotInAlbum = computed(() => {
  return filteredSongs.value.filter((song) => {
    return !albumSongs.value.some((albumSong) => albumSong.ID === song.id);
  });
});
...
const addSongToAlbum = async (songId) => {
  try {
    ...
    fetchAlbum(albumId.value); // Refresh the album's song list
  } catch (error) {
...
/* Song List and Media Player Styles */
.song-list-container {
  margin-top: 30px;
}
