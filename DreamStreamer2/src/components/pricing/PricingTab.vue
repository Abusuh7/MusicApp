<template>
      ...
      <div class="relative flex flex-col h-full p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-900 shadow shadow-slate-950/5">
        ...
        <div class="mb-5">
          ...
          <div class="inline-flex items-baseline mb-2">
            ...
            <span class="text-slate-900 dark:text-slate-200 font-bold text-4xl">{{ props.yearly ? props.price.yearly : props.price.monthly }}</span>
            <span class="text-slate-500 font-medium">/mo</span>
          </div>
  
          <!-- Display the correct plan description based on yearly or monthly -->
          <div class="text-sm text-slate-500 mb-5" v-html="props.yearly ? props.planDescriptionAnnual : props.planDescriptionMonthly"></div>
  ...
  const props = defineProps({
    ...
    planDescriptionAnnual: String, // Description for the annual plan
    planDescriptionMonthly: String, // Description for the monthly plan
    features: Array,
  })
  </script>
